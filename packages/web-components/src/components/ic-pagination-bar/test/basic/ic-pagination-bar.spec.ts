...
describe("ic-pagination-bar", () => {
  ...
  /* removed for now - failing as trimItemsPerPageOptions() has been removed */
  it.skip("should only allow a maximum of 4 custom items per page options", async () => {
    const page = await newSpecPage({
      components: [PaginationBar],
  ...
  /* removed test for now - failing due to resizeObserverCallback being removed from component */
  it.skip("should wrap pagination when the device size is small", async () => {
    const page = await newSpecPage({
      components: [PaginationBar, IcPagination],
      html: `<ic-pagination-bar total-items="100" show-go-to-page-control="true"></ic-pagination-bar>`,
    });
